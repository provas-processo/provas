      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-blue-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="bg-blue-600 p-6 flex justify-between text-white font-bold uppercase"><h2>{currentFolder === null ? "Novo Morador" : "Subpasta"}</h2><button onClick={() => setIsModalOpen(false)}><X /></button></div>
            <form onSubmit={handleCreateFolder} className="p-8 flex flex-col gap-4">
              {currentFolder === null ? (<><input required className="w-full bg-slate-100 p-4 rounded-xl outline-none" value={newResident.nome} onChange={e => setNewResident({...newResident, nome: e.target.value})} placeholder="Nome" /><div className="flex gap-4"><input required className="flex-1 bg-slate-100 p-4 rounded-xl outline-none" value={newResident.bloco} onChange={e => setNewResident({...newResident, bloco: e.target.value})} placeholder="Bloco" /><input required className="flex-1 bg-slate-100 p-4 rounded-xl outline-none" value={newResident.apto} onChange={e => setNewResident({...newResident, apto: e.target.value})} placeholder="Apto" /></div></>) : 
